...
class _TaskListScreenState extends State<TaskListScreen> {
  ...
  Future<void> _loadTasks() async {
    ...
    // Ordenar por dueDate asc (nulos por último), depois por createdAt desc
    tasks.sort((a, b) {
  ...
  Future<void> _toggleTask(Task task) async {
    ...
    // no notification cancellation needed (feature removed)
    await _loadTasks();
  }
  ...
  Future<void> _openTaskForm([Task? task]) async {
    if (task != null && task.dueDate != null && !task.completed) {
      final now = DateTime.now();
      final dueDay = DateTime(task.dueDate!.year, task.dueDate!.month, task.dueDate!.day);
      final today = DateTime(now.year, now.month, now.day);
      if (dueDay.isBefore(today)) {
        // Mostrar alerta de vencida
        await showDialog<void>(
          context: context,
          builder: (context) => AlertDialog(
            title: const Text('Tarefa vencida'),
            content: Text('A tarefa "${task.title}" está vencida desde ${task.dueDate!.day}/${task.dueDate!.month}/${task.dueDate!.year}.'),
            actions: [
